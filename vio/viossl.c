...
static int handle_ssl_io_error(Vio *vio, int ret)
{
  ...
  if (vio->edg_uring && event == VIO_IO_EVENT_READ)
  {
                  ...
                  ssl_handshake_func_t func, unsigned long *errptr)
{
  ...
  // EDG: use uring for incoming connections
  if (func == SSL_accept)
  {
    int edgeless_ssl_use_uring(SSL *, int);
    const int res= edgeless_ssl_use_uring(ssl, sd);
    if (res)
    {
      SSL_free(ssl);
      DBUG_RETURN(res);
  ...
  */
  ...
  if (func == SSL_accept)
    vio->edg_uring= TRUE; // has been cleared by vio_reset
